...
- **India / Nigeria / South Africa** (`india-states.geojson`, `nigeria-states.geojson`, `south-africa-provinces.geojson`) — sourced from **geoBoundaries** ADM1 (the `gbOpen` `_simplified` files, fetched per-country via `https://www.geoboundaries.org/api/current/gbOpen/<ISO3>/ADM1/`). geoBoundaries is the go-to fallback when click_that_hood lacks a country (Nigeria/South Africa) or ships a stale/incomplete one (India). Its ADM1 files are clean and current; India ADM1 is the full 36 (28 states + 8 UTs) with a faithful 22-atoll Lakshadweep. Three post-processing steps: (1) remap `properties.shapeName` → `properties.name` (the renderer matches on `name`); (2) re-simplify at `tol=0.005` for parity with the other hosted files; (3) **re-orient rings clockwise** (see winding note below). geoBoundaries labels carry diacritics (`Bihār`, `Gujarāt`, `Damān`); `normalizeString` strips them, so plain ASCII canonical names match without aliases. Other source quirks do need aliases — e.g. South Africa's file misspells `Northern Cape` as `Nothern Cape`.
- **Indonesia** (`indonesia-provinces.geojson`) — **38** provinces (current). geoBoundaries only carries the pre-2022 **34** (it predates the Papua-region split into Central/Highland/South Papua + Southwest Papua), so this is sourced from a community 38-province dataset (`denyherianto/indonesia-geojson-topojson-maps-with-38-provinces`) instead. Same post-processing: remap the Indonesian `PROVINSI` label → `name`, re-orient rings CW, simplify at `tol=0.005`. Because labels are Indonesian (`Jawa Tengah`, `Papua Barat`, `Daerah Istimewa Yogyakarta`), each record carries the Indonesian form as an alias so the English canonical name resolves.
