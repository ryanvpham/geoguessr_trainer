...
**GeoJSON sources**: most countries pull from `click_that_hood` via jsDelivr (e.g. US, Canada, Brazil, Australia, **India**, **Malaysia**). Note click_that_hood files can be stale: India still carries pre-2019/2020 boundaries (unified Jammu & Kashmir, separate Dadra & Nagar Haveli / Daman & Diu, `Pondicherry` not Puducherry), and its Malaysia file omits Labuan — both handled via aliases / canonical-name choices in `states.js`. Some countries are self-hosted under `public/data/`:
...
- **Indonesia / Nigeria / South Africa** (`indonesia-provinces.geojson`, `nigeria-states.geojson`, `south-africa-provinces.geojson`) — click_that_hood doesn't ship these, so they're sourced from **geoBoundaries** ADM1 (the `gbOpen` `_simplified` files, fetched per-country via `https://www.geoboundaries.org/api/current/gbOpen/<ISO3>/ADM1/`). geoBoundaries is the go-to fallback when click_that_hood lacks a country — its ADM1 files are clean, English-named, and small. Three post-processing steps: (1) remap `properties.shapeName` → `properties.name` (the renderer matches on `name`); (2) re-simplify at `tol=0.005` for parity with the other hosted files; (3) **re-orient rings clockwise** (see winding note below). Source-data quirks become aliases — e.g. South Africa's file misspells `Northern Cape` as `Nothern Cape`.
